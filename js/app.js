...
function resetSidebarLinks() {
    const mainLinks = ['link-dashboard', 'link-pre-orders', 'link-customers', 'link-roles'];
    const submenuLinks = [
        ...
        'link-send-courier', 'link-payments'
    ];
...
function handleRouting() {
    ...
    if (hash === '#/dashboard') {
        ...
        highlightLink('link-inventory', true);
        fetchAllProducts();
        ...
        highlightLink('link-roles', true);
    } else if (hash === '#/admins') {
        container.innerHTML = adminsHTML;
        highlightLink('link-admins', true);
