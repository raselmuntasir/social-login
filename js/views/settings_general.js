...
<div class="space-y-6 pb-10">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <button id="save-general-settings" class="bg-[#7c14b4] hover:bg-[#6a119a] text-white px-8 py-2.5 rounded text-sm font-bold shadow-sm transition-colors mb-2">Update</button>
        <p class="text-[11px] text-gray-900 font-bold">NB: * marked are required field.</p>
    </div>
</div>
`;

window.switchGeneralTab = function(tabName) {
    // 1. Hide all tab contents
    document.querySelectorAll('.general-tab-content').forEach(el => {
        el.classList.add('hidden');
        el.classList.remove('block');
    });

    // 2. Reset all tab link styles
    document.querySelectorAll('.general-tab-link').forEach(el => {
        el.classList.remove('bg-gray-100/80', 'text-gray-800');
        el.classList.add('text-blue-600', 'hover:bg-gray-50');
    });

    // 3. Show active tab content
    ...
    // 4. Set active tab link style
    const activeTab = document.getElementById(`tab-${tabName}`);
    if (activeTab) {
        activeTab.classList.add('bg-gray-100/80', 'text-gray-800');
        activeTab.classList.remove('text-blue-600', 'hover:bg-gray-50');
    }
};
